...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const headers = [
    'part_name', anonymize ? 'participant_id' : 'participant_name', ...(anonymize ? [] : ['participant_email']), 'date', 'submitted_at', 'timezone', 'study_version',
    ...allQuestions.map((q) => `"${q.text.replace(/"/g, '""').replace(/<[^>]*>/g, '')}"`)
  ]
  ...
  const rows = study.parts.flatMap((part) =>
    part.entries.map((entry) => {
      ...
      return [
        ...
        ...(anonymize ? [] : [entry.user.email]),
        entry.date,
        entry.submittedAt.toISOString(),
        entry.timezone ?? '',
        String(study.version),
        ...allQuestions.map((q) => `"${(answerMap[q.id] ?? '').replace(/"/g, '""')}"`),
      ].join(',')
    })
  )
