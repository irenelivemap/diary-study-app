...
test('invite link lets a new participant create an account and join', async ({ page }) => {
  const db = await requirePrisma()
  const study = await loadSimpleStudy()
  const email = `qa.invite.${Date.now()}@diari.test`
...
test('profile keeps a participant on the participant side when going back', async ({ page }) => {
  ...
  await page.getByRole('link', { name: /QA Participant/ }).click()
